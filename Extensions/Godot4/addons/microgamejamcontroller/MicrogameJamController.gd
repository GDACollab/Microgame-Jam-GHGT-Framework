...
func WinGame():
	if not _game_ended:
		_game_ended = true;
		if _is_game:
			JavaScriptBridge.eval('parent.GameInterface.winGame();');
		else:
...
func LoseGame():
	if not _game_ended:
		_game_ended = true;
		if _is_game:
			JavaScriptBridge.eval('parent.GameInterface.loseGame();');
		else:
			print("Game Lost! Restarting...");
			if (_current_scene == get_tree().get_current_scene()):
				get_tree().reload_current_scene();
			else:
				get_tree().change_scene_to_file(_current_scene.filename);
			_game_ended = false;
			_dev_timer = _max_time;
	else:
...
func GetLives():
	if _is_game:
		var lives = JavaScriptBridge.eval('parent.GameInterface.getLives();');
		return lives;
	else:
		return dev_lives;
...
func GetDifficulty():
	if _is_game:
		var difficulty = JavaScriptBridge.eval('parent.GameInterface.getDifficulty();');
		return difficulty;
	else:
...
func GetTimer():
	if _is_game:
		var timer = JavaScriptBridge.eval('parent.GameInterface.getTimer();');
		return timer;
	else:
		return _dev_timer;
...
func SetMaxTimer(seconds):
	if not _game_started:
		...
		if _is_game:
			JavaScriptBridge.eval('parent.GameInterface.setMaxTimer(' + seconds + ');');
		else:
			_max_time = seconds;
	else:
...
func _ready():
	if JavaScriptBridge.eval('parent.GameInterface !== undefined && parent.GameInterface !== null'):
		_is_game = true;
		...
		await get_tree().process_frame;
		JavaScriptBridge.eval('parent.GameInterface.gameStart();');
	else:
		await get_tree().process_frame;
